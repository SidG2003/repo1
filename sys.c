...
SYSCALL_DEFINE4(kernel_2d_memcpy, float*, srcmatrix, float*, dstmatrix, int, num_row, int, num_col)
{
	int flag1;
	int flag2;

	float matrix[num_row][num_col];

	flag1=copy_from_user( &matrix, dstmatrix, sizeof(matrix));
	if(flag1 != 0){
		return -EFAULT;
	}
	
	flag2 = copy_to_user(srcmatrix, &matrix, sizeof(matrix));
	if(flag2 != 0){
		return -EFAULT;
	}
